...
class EducationTrainingBloc {
  ...
  Stream<EducationTrainingEvent> get eventsStream => eventsController.stream;
    ...
    itemService.get(Item(key: EDUCATION_TRAINING_ITEM_KEY)).then((item) {
      ...
      if (achievements.hasData()) {
        ...
        DefaultAssetBundle.of(context)
            .loadString("assets/files/base_data_education_training_screen.json")
            .then((string) {
          var defaultAchievements = Achievements.fromJson(json.decode(string));
          achievementsController.add(defaultAchievements);
          achievementsChangesController.setInitialAchievements(defaultAchievements);
        });
      }
    });
  }
  ...
  void _saveAchievements() {
    print("SAVE Achievements : ${jsonEncode(achievementsChangesController.getCurrentAchievements())}");
    itemService
        .add(Item(
            key: EDUCATION_TRAINING_ITEM_KEY, value: achievementsChangesController.getCurrentAchievements().toJson()))
        .then((item) {
      //TODO Saved finished
    });
  }
